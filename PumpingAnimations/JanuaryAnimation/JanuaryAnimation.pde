PImage January; 
String filename = "January2017CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 31;
  background(255);
  January = loadImage("January2017.png"); 
  imageMode(CENTER);
  image(January, 350, 350, 580, 580);
  nextDay = 0; 
...
{   
  if (nextDay == 31) //restarts everything to the beginning
  {
    ...
    background(255);  
    imageMode(CENTER);
    image(January, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;
  }
