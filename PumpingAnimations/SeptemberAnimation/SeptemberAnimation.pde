String filename = "September2016CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 30;
  background(255);
  September = loadImage("September2016.png"); 
  imageMode(CENTER);
  image(September, 350, 350, 580, 580);
  nextDay = 0; 
...
{ 
  ...
  if (nextDay == allDays.size()) //restarts everything to the beginning
  {
    ...
    background(255);  
    imageMode(CENTER);
    image(September, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;    
  }
