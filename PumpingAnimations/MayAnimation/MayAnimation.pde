PImage May; 
String filename = "May2017CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 31;
  background(255);
  May = loadImage("May2017.png"); 
  imageMode(CENTER);
  image(May, 350, 350, 580, 580);
  nextDay = 0; 
  frameRate(1); //speeds up/slows down the animation
}

void draw()
{   
  if (nextDay == 2 || nextDay == 13 || nextDay == 27)
  {
    i+=360.0/numDays;
  }
  ...
  if (nextDay == 31) //restarts everything to the beginning
  {
    ...
    background(255);
    imageMode(CENTER);
    image(May, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;
  }
