PImage February; 
String filename = "February2017CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 28;
  background(255);
  February = loadImage("February2017.png"); 
  imageMode(CENTER);
  image(February, 350, 350, 580, 580);
  nextDay = 0; 
...
void draw()
{   
  if (nextDay == 28) //restarts everything to the beginning
  {
    nextDay = 0;
    background(255);  
    imageMode(CENTER);
    image(February, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;
  }
