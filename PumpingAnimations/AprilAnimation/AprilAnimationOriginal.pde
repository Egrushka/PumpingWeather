PImage April; 
String filename = "April2017CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 30;
  background(255); 
  April = loadImage("April2017New.png"); 
  imageMode(CENTER);
  image(April, 350, 350, 580, 580);
  nextDay = 0; 
...
void draw()
{    
  if (nextDay == 30) //restarts everything to the beginning
  {
    nextDay = 0;
    background(255);  
    imageMode(CENTER);
    image(April, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;    
  }
