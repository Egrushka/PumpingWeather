PImage October; 
String filename = "October2016CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 31;
  background(255);
  October = loadImage("October2016.png"); 
  imageMode(CENTER);
  image(October, 350, 350, 580, 580);
  nextDay = 0; 
...
void draw()
{   
  if (nextDay == 31) //restarts everything to the beginning
  {
    nextDay = 0;
    background(255);  
    imageMode(CENTER);
    image(October, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;
  }
...
void processData()
{
  ...
  for (int i=1; i<rawData.length; i++)
  {
    ...
    {   
      if (i==1 &&(int(thisRow[5])!=1))//if the first day of the month doesn't start with 1
      {
        p=int(thisRow[5]);
      }   
      for (int m=i-count; m<i; m+=count) //skips by days
