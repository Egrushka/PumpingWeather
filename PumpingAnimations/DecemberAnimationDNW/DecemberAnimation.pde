PImage December; 
String filename = "December2016CSV.csv";
String [] rawData;
...
void setup()
{
  ...
  numDays = 31;
  background(255);
  December = loadImage("December2016.png"); 
  imageMode(CENTER);
  image(December, 350, 350, 580, 580);
  nextDay = 0; 
...
{   
  if (nextDay == allDays.size() ) //restarts everything to the beginning
  {
    ...
    background(255);  
    imageMode(CENTER);
    image(December, 350, 350, 580, 580); //creates a new background to start over
    i = -90.0; 
    count = 1;
  }
